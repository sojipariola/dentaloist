...
class AdminManager:
    ...
    def add_view(self, view, name=None, category=None):
        ...
        if category_name not in self._menu:
            self._menu[category_name] = []
        
        self._menu[category_name].append({
            'name': view.name,
            'endpoint': endpoint,
            'view': view
        })
        
        print(f"✅ Added admin view for {view.name} in category {category_name}")
    ...
    def get_menu(self):
        ...
        # Ensure we always return a proper dict, even if empty
