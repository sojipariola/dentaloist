...
class TestBaseModel:
    ...
    def test_base_model_soft_delete(self, session):
        ...
        class TestModel(BaseModel):
            __tablename__ = 'test_models_soft_delete'
            name = db.Column(db.String(100))
    ...
    def test_base_model_to_dict(self, session):
        """Test to_dict method"""
        class TestModel(BaseModel):
            __tablename__ = 'test_models_dict'
            name = db.Column(db.String(100))
            _secret = db.Column('secret', db.String(100))  # Private field
        
        TestModel.__table__.create(session.bind)
        
        instance = TestModel(name="Test Dict", _secret="hidden")
        session.add(instance)
        session.commit()
        ...
        assert '_secret' not in data  # Private fields should be excluded
...
class TestLookupBaseModel:
    ...
    def test_lookup_model_to_dict(self, session):
        ...
        class TestLookup(LookupBaseModel):
            __tablename__ = 'test_lookups_dict'
            code = db.Column(db.String(50), unique=True, nullable=False)
