...
def get_fernet_key(env_var="ENCRYPTION_KEY") -> str:
    """Retrieve the Fernet key from environment or .env, generate if missing or invalid"""
    ...
    if key:
        try:
            ...
            Fernet(key.encode())
            logger.info("✅ Valid Fernet key found in environment")
            return key
        except (ValueError, Exception) as e:
            logger.warning(f"⚠️ Invalid Fernet key in environment: {e}. Generating a new one...")

    # Generate new valid key
    new_key = Fernet.generate_key().decode()
    logger.info(f"🔑 Generated new Fernet key")
    ...
    try:
        if os.path.exists(ENV_FILE):
            ...
            # Remove any existing invalid key
        ...
        logger.info(f"✅ New Fernet key saved to {ENV_FILE}")
        
    except Exception as e:
        logger.error(f"❌ Failed to save key to {ENV_FILE}: {e}")
    ...
    def init_app(self, app):
        try:
            ...
            if not encryption_key:
                ...
                # Validate the key from config
                try:
                    Fernet(encryption_key.encode())
                except ValueError:
                    logger.warning("Invalid ENCRYPTION_KEY in config, generating new one")
                    encryption_key = get_fernet_key()
            
            self.fernet = Fernet(encryption_key.encode())

            # Handle key rotation if configured
            ...
            if rotation_key:
                try:
                    ...
                    logger.info("Key rotation configured")
                except ValueError as e:
                    logger.warning(f"Invalid rotation key: {e}")
            ...
            if app.debug:
                # In debug mode, allow operation without encryption
                self.fernet = None
                self.initialized = False
                logger.warning("🚨 Running without encryption (debug mode)")
            else:
                raise
...
# ... rest of your existing functions (hash_password, verify_password, etc.)
